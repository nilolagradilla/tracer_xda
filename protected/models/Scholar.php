...
class Scholar extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('lastName, firstName, userId, entryDate', 'required'),
			array('sex, userId', 'numerical', 'integerOnly'=>true),
			array('lastName, firstName, middleName', 'length', 'max'=>100),
			array('nameSuffix', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, lastName, firstName, middleName, nameSuffix, sex, userId, entryDate', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'category' => array(self::HAS_ONE, 'Category', 'scholarId'),
			'tracer' => array(self::HAS_ONE, 'Tracer', 'scholarId'),
		);
	}
...
//-----------------------------------------------------------------------------------------------------------------------------------------------	
	public function search()
	{
		...
		$criteria->with=array(
				"category"=>array('condition'=>'category.statusId=5')
				);
